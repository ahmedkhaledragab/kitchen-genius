...
import { Loader2, Pencil, Plus, Trash2, GripVertical, ChefHat } from "lucide-react";
...
export const Route = createFileRoute("/admin/kitchens")({
  head: () => ({
    meta: [{ title: "إدارة المطابخ — الأدمن" }],
  }),
  component: AdminKitchensPage,
});
...
// The kitchens / ingredient_kitchens tables are new and may not yet appear in
// the auto-generated supabase types. Cast through `any` to keep typecheck calm.
// eslint-disable-next-line @typescript-eslint/no-explicit-any
const sb = supabase as any;

function AdminKitchensPage() {
  const { isAdmin } = useAuth();
  const { lang } = useLang();
  const isAr = lang === "ar";

  const [rows, setRows] = useState<KitchenRow[] | null>(null);
  const [loading, setLoading] = useState(true);
  const [editing, setEditing] = useState<KitchenRow | null>(null);
  const [adding, setAdding] = useState(false);
  const [deleting, setDeleting] = useState<KitchenRow | null>(null);
  ...
  async function handleDragEnd(e: DragEndEvent) {
    ...
    const updates = reordered.map((r) =>
      sb.from("kitchens").update({ sort_order: r.sort_order }).eq("id", r.id),
    );
        ...
        <div>
          <h1 className="text-xl font-extrabold sm:text-2xl">
            <span className="gradient-text">
              {isAr ? "إدارة المطابخ" : "Kitchens"}
            </span>
          </h1>
          <p className="mt-1 text-sm text-muted-foreground">
            {isAr
              ? "كل مطبخ هيظهر للمستخدم في أول الصفحة الرئيسية، ومكوناته بتظهر بعد الاختيار 💕"
              : "Each kitchen shows on the home page; its ingredients appear after selection."}
          </p>
        </div>
      </div>
      ...
      <Card className="rounded-3xl p-4 sm:p-5">
        ...
        {loading ? (
          ...
          <div className="rounded-2xl border border-dashed border-border/60 p-8 text-center text-sm text-muted-foreground">
            {isAr ? "ما فيش مطابخ لسه — ابدأي بإضافة مطبخ." : "Nothing yet — add your first kitchen."}
          </div>
        ) : (
        ...
        <AlertDialog open={!!deleting} onOpenChange={(o) => !o && setDeleting(null)}>
          <AlertDialogContent className="rounded-3xl">
            <AlertDialogHeader>
              ...
              <AlertDialogDescription>
                {isAr
                  ? `هتمسحي مطبخ "${deleting ? deleting.name_ar : ""}"؟ ده هيشيله من المكونات المربوطة بيه.`
                  : `Delete kitchen "${deleting?.name_en ?? ""}"? This cannot be undone.`}
              </AlertDialogDescription>
            </AlertDialogHeader>
            <AlertDialogFooter>
              ...
              <AlertDialogAction
                ...
                onClick={async () => {
                  if (!deleting) return;
                  const { error } = await sb.from("kitchens").delete().eq("id", deleting.id);
                  if (error) {
...
function SortableRow({
  ...
  return (
    <li ref={setNodeRef} style={style} className="flex items-center gap-2 py-2.5">
      <button
        type="button"
      ...
      <div className="min-w-0 flex-1">
        <div className="flex items-center gap-2">
          <p className="truncate text-sm font-bold">{isAr ? row.name_ar : row.name_en}</p>
          {!row.is_active && (
...
function KitchenDialog({
  ...
  const [descAr, setDescAr] = useState(row?.description_ar ?? "");
  ...
  async function handleSave() {
    ...
    const { error } = isEdit
      ? await sb.from("kitchens").update(payload).eq("id", row!.id)
      : await sb.from("kitchens").insert(payload);
    setSaving(false);
    if (error) {
      toast.error(
      ...
      <DialogContent className="max-h-[90vh] overflow-y-auto rounded-3xl">
        ...
        <div className="grid gap-3 py-2">
          ...
          <div className="grid gap-1.5">
            ...
            <Input value={nameAr} onChange={(e) => setNameAr(e.target.value)} placeholder="مصري" />
          </div>
          ...
          <div className="grid gap-1.5">
            <Label>{isAr ? "وصف قصير (عربي)" : "Description (AR)"}</Label>
            <Input
              value={descAr}
          ...
          <div className="grid gap-1.5">
            <Label>{isAr ? "وصف قصير (إنجليزي)" : "Description (EN)"}</Label>
            <Input
              value={descEn}
          ...
          <div className="flex items-center justify-between rounded-2xl border border-border/60 p-3">
            <Label className="text-sm">{isAr ? "مفعّل" : "Active"}</Label>
